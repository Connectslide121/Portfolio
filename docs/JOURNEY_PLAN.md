...
| D31 | An **isometric overview** (`O`, or the Overview button) shows all seven beats at once on a valley path, threaded by the same heat-gradient stream, each plinth clickable to jump | The linear stage answers "where am I"; this answers "how far is it, and what shape was it". Straight-line and true-grid layouts both came out looking like the rail again |
| D32 | Below 900px the overview drops the map and becomes a **tinted vertical list** | An isometric map needs width a phone has not got, and absolutely-placed labels detach from a horizontally scrolling map. The list still delivers the point — the whole arc in one view |
