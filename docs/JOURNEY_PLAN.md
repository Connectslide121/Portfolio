...
| D45 | **Reverses D13.** A pick off the map **travels**, on a fixed short budget (`0.45s + 0.1s/beat`, capped ~1.05s, `power2.inOut`) rather than seeking instantly | D13's reasoning was that picking a year should not replay the two decades in between — but at the timeline's natural rate, which is 2.4s per beat. A fixed budget keeps the road visible without dwelling on any of it. Stepped moves still scrub at the natural rate, so they stay in sync with the beat they belong to |
