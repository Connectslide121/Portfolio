...
| D49 | The Sprinta beats carry a **row of product links**, and every product URL lives once in `productUrl` | The journey said what was built but gave no way to go and look, which the résumé bullets always had. The card has no room for bullet prose, so the products stand on their own as links riding `--j-accent`. The URLs were about to exist in three literals (bullet links, summary links, journey cards) — the display label still varies by context, only the address is shared |
| D50 | **Depth is carried by air, not by blur.** A receding place blends toward `--j-sky1` as well as losing opacity; a haze band covers the two ambient ridges; silhouettes fill with a gradient drifting toward `--j-far` at the top | Fading toward transparent reads as a thing being removed; losing contrast into the horizon is what the eye reads as distance. Blur was rejected on cost — the far layer is `SCENE_W * BEATS.length + OVERDRAW` ≈ 15,800 units wide and blurring it rasterises an enormous texture — and on merit: defocus is a camera artefact, aerial perspective is physics |
| D51 | **Each scene carries its OWN copy of the silhouette gradient** (`jSil-<beatid>`), never one shared def | A `var()` in a gradient stop resolves against the gradient ELEMENT's inherited value, not against whatever references it. A def in the root `<defs>` always reads the root's `--j-mid`, so every per-scene override the render loop writes — the overview's own-heat blend, the depth haze — would silently do nothing. This is the single easiest thing in the file to get wrong |
| D52 | **Light shafts are ellipses with a radial gradient focused at the mouth**, not cone polygons | A polygon's sides stay hard however soft its gradient is. The first pass came out as origami: four flat triangles meeting at a point above the India dome. A radial fade in every direction is also what lets overlapping shafts blend into a glow instead of stacking as visible facets |
| D53 | **The camera's breath is derived from the fractional camera position, never tweened per beat** — pulled back and lifted while travelling, settled on arrival | Derived, it scrubs correctly in both directions, at any speed, from any starting point. A tween bolted onto each beat segment would have to be unwound by hand on the way back. It rests at 1 and pulls BACK rather than pushing in, so arrival never crops tighter than the framing `CURRENT_SCALE`/`CURRENT_ANCHOR` were tuned against |
| D54 | **Pointer lean runs on its own rAF that sleeps when there is nothing left to close**, not on the GSAP ticker | Journey mode is the default landing (D17). A permanent 60fps render loop on it is a battery bill charged to every visitor, including the ones who never touch it. Touch is excluded entirely (`pointer: fine`) — a coarse pointer only reports mid-gesture, so it would lurch on every swipe |
| D55 | Grain and a vignette go over the **world only, never the cards** | Flat vector reads as clip art largely because it has no surface — a mathematically even colour is a thing that occurs in no photographed or printed image. Noise over text is the fastest way to make type look cheap |
| D56 | **The globe is SVG trigonometry, not three.js** — and it is a secondary layer, half-sunk behind the map, hidden at the recap | It is the one part of the journey with a real claim on 3D and the one that least needs an engine: no model, no texture, no lighting, and an orthographic projection of a sphere is four lines of maths. As SVG it also inherits the heat variables, so it rides molten → cold for free. WebGL would have cost more than the entire journey bundle (≈110–150 kB gz against 14.7 kB) and would have had to be recoloured by hand. Hidden at the recap because the bird's-eye layout (D31) is the same idea done better, and two maps of one journey is one too many |
| D57 | The globe is driven by the **continuous camera position**, and its viewpoint is **lifted 36° south of the current place** rather than centred on it | Driven from the camera it turns with the travel instead of snapping when the travel ends, and scrubs backwards without being told. Centred, the active dot lands in the middle of the disc — which is exactly where the map sits, the one part of the globe that cannot be seen |
| D58 | Globe smoothness came from **`contain: paint`, not from throttling** | The update is ~0.2ms of maths; the cost is repaint. Skipping frames did not remove that cost, it doubled it on the frames that did run and turned an even load into visible hitching. Confining the invalidation to the globe's own box put the transition back within noise of not having a globe at all. Measured both ways before choosing |
| D59 | The globe carries **coastlines, not country borders**, from Natural Earth 110m land decimated to ~870 points | The ask was for "countries", but at this radius Europe is about forty pixels across and 110m borders inside it are an illegible scribble rather than a hint. Coastlines are the shapes that survive the scale. Outer rings only (holes are invisible here), anything under 1 square degree dropped, Douglas-Peucker at 0.9° — chosen against the RADIUS, where 1° is ~2.2 units — and rounded to 0.1°. 5,143 points → 873, +4.1 kB gz |
| D60 | Land is **stroked, never filled**, and in **one thin pass** | Filling needs a ring crossing the horizon to be closed along the LIMB — the fiddly half of a spherical clip — and closing across the chord instead cuts a straight edge through the disc. A wide soft underlay was then tried beneath the coast to suggest mass, and measured as the globe's entire frame cost: ~870 points at 5 units roughly doubled dropped frames, while the same path at 1.25 units is within noise of drawing no land at all |
| D61 | **No far-side ring rejection.** Written, measured, removed | The obvious optimisation — skip rings whose bounding cap is entirely behind the globe — saved nothing: `strand()` already discards a hidden point for about eight multiplies. The globe's cost is stroke rasterisation, not projection (the whole update is ~0.5ms). Recorded so it is not "optimised" back in |
| D48 | The **ease belongs to the playhead, not to each beat**. The per-beat camera and stream tweens are `ease: "none"`; every driver's `tweenTo` carries `power2.inOut` | Each beat's camera tween used to ease in *and* out, so the camera came to a standstill at every segment boundary — a pick that crossed six of them stopped six times on the way. With the segments linear and equal (`SCENE_W` over 2.4s each), velocity is continuous across boundaries, and a single step still gets exactly the motion it had because one ease over one segment is the same curve |
| D45 | **Reverses D13.** A pick off the map **travels**, on its own budget (`0.6s + 0.28s/beat`, `power2.inOut`) rather than seeking instantly | D13's reasoning was that picking a year should not replay the two decades in between — but at the timeline's natural rate, which is 2.4s per beat. Weighting the budget toward distance keeps the road watchable — a neighbour is a hop, 2005 -> today is ~2.5s against ~17s at the natural rate — without dwelling on any of it. Stepped moves still scrub at the natural rate, so they stay in sync with the beat they belong to |
| D46 | The exit button needs an explicit `pointer-events: auto` | `.j-topbar` is `pointer-events: none` so the world stays live under it, and only the theme toggle opted back in. "Classic view" was therefore unclickable and the Escape key was the only way out of journey mode — which, given journey mode is the default landing (D17), meant most visitors had none |
| D47 | On the recap the map **keeps its labels** and drops flush to the frame edge instead; recap labels that would land under it are pinned above it by their `bottom` edge (`data-lifted`) | First tried collapsing the map to dots up there, since the laid-out places carry labels of their own — it read as the navigation breaking on the last slide. Anchoring the lifted ones by `bottom` means CSS still decides their height, so nothing has to guess it. The drop is a 1.1s transition: the camera lift it accompanies runs 2.4s, and a short front-loaded ease finished long before the move it belonged to |
| D40 | The bottom rail is a **map, not a page counter**: every stop carries a short tag ("Steel foundry", "Career reset") alongside its year, threaded on a line whose gradient is each beat's own heat | A year alone orients only someone who already knows the story — which a first-time recruiter does not. They opened on a software CV, hit "Engineering Studies" and a foundry, and had nothing on screen telling them that was the setup. The arc is now legible at every beat rather than only at the recap, and the strip is the navigation |
| D41 | The landing beat leads with **three acts** — Steel / The reset / Software — above the CTA, and the long profile blurb is replaced by a one-line hook (`profile.lede`) | The ask was explicitly "not a pure text paragraph, people don't read". The acts are coarser than the map below them on purpose: the map lists stops, these name chapters, so the two do not duplicate. `profile.blurb` is untouched — the static hero still uses it |
| D42 | **Reverses D36.** The exit reads **"Classic view →"** | "Static page" is a developer's word for it. D36's reasoning still holds — the button switches presentation, not content — so the replacement stays mode-flavoured rather than becoming "View the full CV" |
| D43 | From above, **each place is painted in its own heat**, not the current one. `heatPalette()` gives the whole ramp at any heat; `projectScenes`' render pass blends each scene's `--j-mid/ground/stream/streamCore/accent` toward its own as `camera.overview` rises, and clears the inline overrides on the way back down | `applyHeat` ramps one heat across the whole stage, which is right while travelling — a receding place should sit in the current evening's light. Laid out, it painted twelve years of molten steel as a cold Swedish afternoon. Sky and ground stay shared: from above there is one sky, but each place keeps its own light |
| D44 | The map collapses to **dots only on the recap** (and on phones) | On the recap the places are laid out with labels of their own; repeating them in the rail crowded the bottom of the frame those labels need. The rail is `<div role="navigation">`, **not** `<nav>` — `navbar.css` styles the bare `nav` element (fixed to the viewport top, 80px, space-between) and the site's own navbar has no class to scope that rule to |
...
| D62 | **On phones the recap is not a wall and not two panels — it is the work, listed** (reverses D32 and the mobile half of D23/D26) | Below 900px the scattered wall had already collapsed into two scrolling columns of 8rem tiles, which showed neither a screenshot nor a clip, with the journey's own tinted list scrolling under it: two cramped panels fighting for one slide. The list gives each project a title, a line on what it is, its stack and its links — the only parts a phone reader can act on — and takes the whole slide. The overview is hidden there rather than restyled: the rail below already names every stop and jumps to it. No `<video>` is mounted on a phone at all, since the tiles are not rendered rather than merely hidden |
| D63 | The phone list's rows carry the **clip's own first frame as a still**, and the row **is a button** that opens the project as a **sheet** where the clip plays | Text-only rows lost the thing a portfolio is for: the work is visual. But a thumbnail big enough to read is a thumbnail you can only fit three of, so the row shows a still and the sheet shows the clip at a size worth watching. The still is the video itself with a `#t=0.1` fragment rather than a poster file — no second asset to keep in step, and without the fragment a poster-less video paints black on mobile Safari. The row has no links in it: a thumb aiming at a 0.68rem pill hits the row anyway, so the row is one target and the links live in the sheet at 0.78rem with real padding. Stills stand down while a sheet is open — a phone's video decoders are a small shared pool and the one clip actually playing should have it |
| D64 | The projects' technology table moved to **`src/data/projectTech.js`**, shared by Résumé mode and the work sheet | Same reasoning as D24 for media: the key→{icon, name} map lived inside `Projects.jsx`, and copying it into the journey would have guaranteed drift the first time a project gained a technology. Keyed by the short names the projects data has always used (`csharp`, `ai`), which is a different vocabulary from `TECH_LOGOS` in `techLogos.js` — that one is keyed by the label printed on a stack pill ("C# / .NET"). Same images, two vocabularies, deliberately not merged |
| D65 | Phones get **labelled Back / Next buttons under the map**, and a swipe now works **on whichever axis the thumb actually moved** | The wheel is the desktop driver and phones have no wheel; the rail's arrows are dropped below 768px for room; which left a horizontal swipe as the only way forward, and nothing on screen said so. Vertical is the axis a phone reader reaches for first — every other page on their screen moves that way — and up now means "on", except where a gesture starts inside something that scrolls (the work list, an open sheet), which keeps its own drag. The buttons sit under the map rather than under the topbar: that is where the journey's navigation already lives, and the top of a phone is the hardest place on it to reach |
| D66 | Project clips are **fetched once, and never before they are wanted**: one URL per clip, `preload="none"` on the résumé's cards until they are within a screen, `preload="metadata"` on the journey's list stills | Journey mode is the default landing and it covers the résumé page — which was mounting four `autoPlay` clips behind it, so the site pulled tens of megabytes nobody could see. The journey's stills then asked for the same clips under a **different URL** (`#t=0.1`, to force a poster frame), which is a second fetch of the same bytes; the frame is now painted by seeking in JS instead, so the sheet reuses what the still pulled. Measured with Chrome's own net log: the CV landing went from four autoplaying clips to **0 bytes** of video, and the recap's six stills cost 13.9 MB. **That 13.9 MB is the next thing to fix and it is not a code problem** — none of the nine files is `faststart` (`ftyp free mdat moov`), so a player fetching *metadata* pulls ~2.5 MB from the front, fails to find the index, and issues a second request for the tail. `ffmpeg -c copy -movflags +faststart` fixes it losslessly. The files are also simply enormous — 238 MB for nine 720p screen recordings, 74 MB for one — which no loading strategy can hide |
| D67 | **The wheel stays one gesture, one beat.** A scroll-linked scrub was built on `feat/journey-scroll-scrub` and rejected on feel | The playhead followed the wheel directly — as far as you scrolled, stopping mid-beat wherever you stopped, with a named move in flight yielding instantly to the hand. It read as loose rather than responsive: a story told in seven framed compositions wants to arrive at them, and a beat left half-crossed is a frame nobody composed. Recorded so it is not rebuilt from the same reasoning. The mobile half of the same complaint was real and stays fixed (D65) |
| D68 | The 2024 beat is a **plot, not a house**: picket fence, gateposts, shrubs, and a cherry tree that **dresses for the same season as the weather** | It was the one beat that did not fill the frame — a single tall block where every other place spans it corner to corner. The tree stands to the RIGHT of the house, which is not where a garden would naturally go: the globe (D56) is drawn over the world and sits dead centre in the frame, exactly where the open ground to the left lands, so a tree there was a shape behind a wireframe. Cherry, not another conifer — the pines two beats earlier are triangles, so this is a cluster of circles wider than it is tall, which is the whole difference. It reads the season off `SEASON_AMBIENT`: pink blossom in spring, thinning amber in autumn, a plain canopy in summer, bare boughs in winter. A cherry in full bloom under falling autumn leaves is two scenes at once. The fine branches exist only for winter, where the first pair alone read as a slingshot |
| D69 | **No move waits its turn.** The lock that dropped input during a transition is gone; a second scroll retargets from wherever the playhead has reached, and a stepped move is capped at 2.4s so chained steps do not stack | This is the other half of the complaint D67 answered wrongly. The feel was never the one-gesture-one-beat model — it was that a move held a lock for its whole duration *plus* a settling window, so for about two and a half seconds after every step the journey ignored you, and the second scroll of a pair did nothing. Interrupting is free here because a stepped move measures its duration from `tl.time()`, so retargeting mid-flight is already a shorter move; the cap is what stops each chained step being longer than the last and the story falling behind the hand. **Its ceiling is the segment about to be traversed, not a constant** — a fixed 2.4s cap was a regression: beats are not all the same length in timeline seconds. Measured from `tl.labels`, every beat is 2.6s except **architect at 3.85s**, which carries the pull-back, the connector draw and the staggered node build, so the cap played it at 1.6x its authored speed and it read as abrupt. Note this only reproduces at desktop width: the whole architect block is gated on `!narrow`, so a measurement taken in a small window shows an innocent 2.6s. Measured that way a single step is always exactly its own segment, while a chained step still covers more ground in one segment's time, which is the catching up it is for. The wheel's own rhythm replaces the lock: `STEP_GAP` 300ms between beats, `FIRST_NUDGE` 12px to start from rest, `NEXT_NUDGE` 48px to take another beat without the gesture pausing — the last one is what keeps a trackpad's inertial tail from spending three beats on one flick. All four are named constants at the top of the file, because they are feel and feel gets tuned |
| D70 | The landing beat's image is **a pour, authored in SVG**: ladle -> pipe -> foundry in Spain -> plant in India -> a mould in Sweden that casts a developer at a desk. The globe turns through the same three places on the same clock | It was a 3D render borrowed from the résumé hero — the only raster in journey mode and the only thing in it not authored in code (D7), so it read as a different product bolted to the front of this one, and it said nothing. The pour states the whole thesis before a word is read, and the cavity is deliberately the figure from the 2024 beat, which is also what the old render showed: a person at a desk was the one thing about it worth keeping. The pipe **fills and then drains** rather than passing a bead along: with `pathLength="100"` the visible run is `stroke-dasharray: <len> <gap>` starting at `-stroke-dashoffset`, so animating both together moves the head and the tail independently — head 0→100 with the tail held (filling), then the tail 0→100 with the head held (emptying into the mould). The gap is 200, wider than the whole path, or the dash pattern repeats and a phantom second run appears behind the first. A place the metal has passed **stays lit until the ladle tips again**: the image is a route being lit one stop at a time, and stations that faded back out threw that away. **All of it is CSS on one shared cycle** with percentage keyframes rather than a stack of delays — delays drift, and a page left open overnight ends up filling the mould while the ladle is still upright. No timeline and no rAF, because this is the default landing beat (D54). The globe is the one exception and needs rAF: it walks the same waypoints as the pour's keyframes, **looping with it**. It was built to sweep once and park, to keep a permanent loop off the landing beat (D54) — but the pour behind it keeps running, so from the second cycle on the globe sat still while the metal moved, which reads as broken rather than as thrifty. The loop only lives while the intro is the beat on screen and rAF does not fire for a hidden tab, so the cost is bounded by someone actually watching it. The wrap is seamless because beat 0 and beat 5 are the same coordinates. The globe **reads the pour's own animation clock** (`getAnimations()` on the charge path) rather than keeping a timer beside it: two clocks started at different moments and restarted at different moments — leave the landing beat and come back and the globe's timer began again while the animation, which never stopped, was wherever it was. `PREVIEW_CYCLE` survives only as the fallback pace for narrow screens, where the art is `display: none` and a hidden element's animations do not progress, so there is nothing to be in sync with. The viewBox carries **margin on every side**: the glows are wider than the things that cast them (a flare's own radius plus three sigma of its blur), and both the viewBox edge and the `contain: paint` that keeps the repaint cheap clip at the same boundary — so India's halo came out sliced down its left side until the margin went into the coordinates |
| D71 | The render loop **publishes the pointer lean as `--j-lean-x` / `--j-lean-y` on the stage**, and the pour reads it | The world is SVG and gets its tilt through the layer setters; the pour is real DOM beside the card and would otherwise have needed a pointer listener of its own — a second one, with its own rAF, on the default landing beat. Reading the same signal means it inherits D54's rules for free: the loop sleeps when there is nothing left to close, and a coarse pointer never starts it. Published in world units scaled by 0.1, so a consumer decides what a unit is worth. The parallax goes on WRAPPER groups, never on the animated elements — the ladle tips with a transform of its own, and two transforms on one element is the animation winning and the parallax silently doing nothing |
| D72 | In daylight the globe gets a **faint dark face**, rather than restyled lines | It is half-sunk: the top half sits on a pale sky and the bottom half on the dark ground, so no single ink serves both — bright lines vanish above the horizon and dark ones vanish below it. A translucent disc puts the same graticule on the same ground in both halves, which is what a planet ought to have anyway, and leaves the heat ramp driving the line colours untouched |
| D73 | The stage is sized with **`100dvh`**, not by `inset: 0` alone | A fixed `inset: 0` element is sized to the SMALL viewport — the one with the phone's URL bar showing. A drag inside the work sheet is enough to retract that bar, the visual viewport grows, the stage does not, and a white strip appears under the journey and stays for as long as the bar is hidden. It looked like the sheet leaving something behind; it is the viewport, and the sheet only triggered it. `dvh` tracks the bar in both directions, with a `vh` line above it for browsers that lack it. The body scroll lock in App.jsx was already doing its job and was never the problem |
...
1. **Skill tree** replacing the `Tools.jsx` icon grid. Nodes unlock
   ...
   hover shows *"acquired 2024 · shipped in Knowia Studio"*. Better information
   design than the grid *and* it reads as a game.
2. **Command palette (Ctrl+K)** — `whoami`, `goto 2017`, `open knowia`.
...
Also rejected, after being asked for directly (2026-09-13):

- **Scroll-scrubbed frame sequences**, the Apple product-page technique. It is
  **D7's problem multiplied by the frame count**: 100–300 baked JPEGs per beat
  across eight beats is tens of MB on GitHub Pages, and a baked pixel cannot be
  recoloured by `camera.heat`, cannot be projected toward `VANISH` as it
  recedes, and cannot be laid out bird's-eye at the recap. The journey is also
  beat-stepped, not scrub-driven, which is the one thing that technique needs.
- **three.js with per-beat 3D models.** Roughly doubles the JS payload before a
  single model loads, and the models do not exist — sourcing them reads as
  asset-store, making them is weeks of Blender. The deeper problem is
  stylistic: the world coheres because it is one flat silhouette palette
  recoloured by one number, and dropping lit, shaded geometry into that is two
  art directions fighting. See **D56** for where a globe differs.
...
|---|---|---|
| 2026-09-14 (7) | Phone and daylight fixes on the new work sheet: the white strip under the journey after opening a project was the mobile URL bar, not the sheet (D73); the clip's letterbox no longer paints a black bar across a white sheet; the theme toggle moved hard right, where the hidden brand had been leaving it stranded on the left. The globe now reads the pour's own animation clock instead of keeping a timer beside it. | **Phase 4**; **O2** Vite; **O4**; **re-encode the project clips** |
| 2026-09-14 (6) | Replaced the landing beat's 3D render with **the pour** (D70), then reworked it to fill the pipe and drain it into the mould rather than run a bead down it, with each place staying lit once passed. Gave it pointer parallax off the world's own lean (D71): molten steel out of a ladle, down a pipe through the foundry in Spain and the plant in India, into a mould in Sweden that casts a developer at a desk. All CSS on one shared cycle; the globe walks the same three places once and parks. `home-image.webp` is now résumé-only. | **Phase 4**; **O2** Vite; **O4**; **re-encode the project clips** |
| 2026-09-14 (5) | Scroll responsiveness, properly this time (D69): the transition lock is gone, so a move in flight is always interruptible and a second scroll retargets instead of being swallowed. Stepped moves capped so chained ones do not stack. The wheel's feel now lives in four named constants. | **Phase 4**; **O2** Vite; **O4**; **re-encode the project clips** |
| 2026-09-14 (4) | Tried scroll-linked scrubbing on `feat/journey-scroll-scrub` — the playhead following the wheel directly, stopping mid-beat — and reverted it on feel (D67); branch deleted, reasoning kept. Then gave the 2024 beat a garden: fence, gateposts, shrubs and a seasonal cherry tree, so it fills the frame like every other place (D68). Scenes now receive the active season. | **Phase 4**; **O2** Vite; **O4**; **re-encode the project clips** |
| 2026-09-14 (3) | Phone navigation and video loading. Stepping got **Back / Next buttons under the map**, and a swipe now reads whichever axis the thumb moved — vertical did nothing before, which is why scrolling "did not work" on a phone (D65). The intro's three acts line up again: dates down the left, names flush right. Clips are now fetched **once, when wanted** (D66) — the CV landing pulls 0 bytes of video where it used to autoplay four, and the journey's stills and sheet share one URL. Left open: the files themselves, 238 MB across nine, none of them faststart. | **Phase 4**; **O2** Vite; **O4**; **re-encode the project clips** |
| 2026-09-14 (2) | The phone list got its pictures back (D63): every row now carries the clip's first frame, and tapping a row opens the project as a sheet — the clip playing, the full description, the stack with its marks, and links at a tappable size. Opening one parks the journey's drivers, so nothing steps behind it, and Escape closes the sheet rather than the whole journey. The technology table behind those marks moved out of `Projects.jsx` into shared data (D64). | **Phase 4** game mechanics; **O2** Vite; **O4** |
| 2026-09-14 | **The recap, on a phone** (D62). The wall degraded badly below 900px — two columns of tiny tiles above a second scrolling list — so narrow screens now get the six projects as a plain list that owns the slide, and the overview is hidden there. Wheel over the list scrolls it before it steps the journey, and a swipe now needs horizontal intent so scrolling the list no longer throws the reader onto another beat. | **Phase 4** game mechanics; **O2** Vite; **O4** |
| 2026-09-13 (2) | Globe got **real coastlines** (D59–D61) — it read as a wireframe, not as Earth. Natural Earth 110m land, decimated to 873 points, stroked in one thin pass. Journey chunk 14.7 → 22.0 kB gz for the whole of this session's work. | **Phase 4** game mechanics; **O2** Vite; **O4** |
| 2026-09-13 | **Depth and cinematography pass, plus the globe** (D50–D58), on `feat/journey-depth-and-globe`. The art was fine; the presentation was flat — even fills, uniformly sharp layers, one focal length throughout. Added aerial perspective on receding places, a horizon haze band, gradient silhouettes, light shafts, a camera that breathes with the move, pointer lean, grain/vignette, an arrival bloom and numerals that are driven home rather than crossfaded. Then the globe behind the map: orthographic SVG, spun by the camera. +3.1 kB gz for all of it. Three.js and Apple-style frame sequences were both considered and rejected — see D56. | **Phase 4** game mechanics; **O2** Vite; **O4** |
...
| 2026-09-13 (4) | Product links added to the Sprinta beats, with every product URL centralised in `productUrl` (D49). "Sole architect" reworded throughout — he leads a team. | **Phase 4**; **O2** Vite; **O4** |
| 2026-09-13 (3) | Travel on a map pick slowed (weighted by distance), then fixed properly: the per-beat ease was making the camera stop at every boundary it crossed, so the ease moved to the playhead (D48). | **Phase 4**; **O2** Vite; **O4** |
| 2026-09-13 (2) | Follow-ups: map picks now travel instead of teleporting (D45, reverses D13); fixed the exit button, which had never been clickable (D46); the map keeps its labels on the recap and drops flush instead, with colliding recap labels lifted above it (D47). Landing acts trimmed to one word each. **Larademy renamed to Knowia Studio** throughout. | **Phase 4**; **O2** Vite; **O4** |
| 2026-09-13 | Branch `feat/journey-navigation-overhaul`. Replaced the year strip with a labelled **journey map** (D40) and gave the landing beat **three acts** instead of a paragraph (D41), both aimed at the same failure: a recruiter arriving on a software CV and hitting a steel foundry with nothing telling them why. Relabelled the exit (D42, reverses D36). Laid-out places now carry **their own heat** (D43) — the first three read molten again. Map collapses to dots on the recap (D44); progress stream raised to clear it. Verified at 1440x900, 1440x760, 1024x700 and 390x780, both themes. | **Phase 4**; **O2** Vite; **O4** |
| 2026-09-11 (9) | Added a temporary **season picker** to the journey topbar to preview all four season ambients, then removed it again once seen — ambience stays calendar-driven. Kept the fix it exposed: particle tweens were paused at creation for every scene but the intro, and the timeline only re-asserts activation when the active scene *changes*, so a mid-journey ambient switch froze the current scene's weather. The active scene index now lives on the stage's `data-active-scene`, and recreated particle groups sync against it. Also made the whole journey theme toggle ambient-aware (track, border and glyphs ride `--j-accent`, not just the thumb). | **Phase 4**; **O2** Vite; **O4** |
